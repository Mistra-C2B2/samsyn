    ...
    try {
      ...
      const newFeatures: Feature[] = geoJsonData.features.map(
        (feature: any) => ({
          ...
          coordinates: feature.geometry.coordinates,
          icon:
            (feature.properties.icon as IconType) || "default",
          lineStyle:
            (feature.properties.lineStyle as LineStyle) ||
            "solid",
        }),
      );
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <Tabs defaultValue="draw" className="w-full">
          ...
          >
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-slate-700">
              <p>
                Paste a GeoJSON FeatureCollection. Each feature
                will be imported as a separate item that you can
                name and customize.
              </p>
            </div>
            <Textarea
        ...
        {features.length === 0 && (
          <div className="p-6 bg-slate-50 border border-slate-200 rounded-lg text-center">
            <p className="text-sm text-slate-600">
              Click a button above to start drawing features on
              the map
            </p>
          </div>
        )}
        ...
        <div className="space-y-3 border-t border-slate-200 pt-4">
          ...
          <p className="text-xs text-slate-500">
            You can change this setting later as the layer
            creator
          </p>
        </div>
      </div>
