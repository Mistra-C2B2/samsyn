...
export function TimeSlider({ startDate, endDate, currentRange, onRangeChange }: TimeSliderProps) {
  ...
  useEffect(() => {
    const startPosition = currentRange[0].getTime() - effectiveStartDate.getTime();
    const endPosition = currentRange[1].getTime() - effectiveStartDate.getTime();
    const newStartValue = (startPosition / totalDuration) * 100;
    const newEndValue = (endPosition / totalDuration) * 100;
    setSliderValues([Math.max(0, Math.min(100, newStartValue)), Math.max(0, Math.min(100, newEndValue))]);
  ...
  const handleSliderChange = (values: number[]) => {
    ...
    // Snap to scale
    if (scale === 'months') {
      newStartTime = new Date(newStartTime.getFullYear(), newStartTime.getMonth(), 1);
      newEndTime = new Date(newEndTime.getFullYear(), newEndTime.getMonth() + 1, 0); // Last day of month
    } else if (scale === 'years') {
  ...
  const hasCustomRange = customStartDate.getTime() !== startDate.getTime() || 
        ...
        <div className="flex-1">
          ...
          <div className="flex justify-between mt-1.5 text-[10px] text-slate-500">
            ...
            <div className="flex gap-1.5 bg-teal-50 px-2 py-0.5 rounded">
              <span className="font-medium text-teal-700">{formatDate(currentRange[0])}</span>
              <span className="text-teal-400">→</span>
              <span className="font-medium text-teal-700">{formatDate(currentRange[1])}</span>
            </div>
