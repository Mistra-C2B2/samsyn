...
export class MapService {
  ...
  async listCollaborators(mapId: string) {
    return this.client.get<MapCollaboratorResponse[]>(`/api/v1/maps/${mapId}/collaborators`);
  }
  ...
  async addCollaborator(
    ...
    const data: MapCollaboratorCreate = { user_id: userId, role };
  ...
  async removeCollaborator(mapId: string, userId: string) {
    return this.client.delete<void>(`/api/v1/maps/${mapId}/collaborators/${userId}`);
  }
  ...
  transformToUserMap(mapResponse: MapResponse) {
    return {
      ...
      permissions: {
        editAccess: mapResponse.edit_permission as 'private' | 'collaborators' | 'public',
        collaborators: mapResponse.collaborators.map(c => c.user_id),
        visibility: mapResponse.view_permission === 'public' ? 'public' : 'private',
      },
    };
  }
